        ...
        for move_number in range(K+1):
            ...
            if move[0] == "E":
                ...
                if lo <= estimate and estimate <= hi:
                    print(
                        f"Case #{case_number}: Correct -- got {estimate}; exact answer is {correct_total_edges}.", file=sys.stderr)
                    correct += 1
                else:
                    print(
                        f"Case #{case_number}: Wrong -- got {estimate}; exact answer is {correct_total_edges}; acceptable range is [{lo}, {hi}].", file=sys.stderr)
